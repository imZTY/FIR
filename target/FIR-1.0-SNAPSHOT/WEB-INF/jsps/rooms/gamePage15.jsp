...
<div class="giveUp">
    <p>投降</p>
    <div class="rootId" id="${blackId}"></div>
</div>
<script>
    //这句话千万不能少 ，表示允许使用推送技术
    ...
    function pageFallChess15(row,col) {
        if (maps[row][col] === 0) {
            if (isBlack) {
    ...
    var row, col, index = 0;
        ...
        can.onclick = function play(e) {
                if(checkROOT(rootId)){
                    ...
                    if (maps[row][col] === 0) {
                        DwrPush.changeROOT(${rootId},${roomId});
                        DwrPush.fallChess(row,col,${roomId});
                    }
                }else {
        ...
        function celebrate() { //显示哪边赢
            ...
            if (total >= 5) {
                if (t == 1) {
                    ...
                    //未避免重复存储，用判断来规定当前用户只存储自己的记录
                    if (${rootId}==${firGameUserA.userId}){
                        DwrPush.addRecord(${firGameUserA.userId},1,maps);
                        DwrPush.updateUser(${firGameUserA.userId},1);
                    }else{
                        DwrPush.addRecord(${firGameUserB.userId},-1,maps);
                        DwrPush.updateUser(${firGameUserB.userId},-1);
                    }
